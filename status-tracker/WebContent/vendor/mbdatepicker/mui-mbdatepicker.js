  ...
  app.directive('mbDatepicker', [
    '$filter', function($filter) {
      return {
        ...
        templateUrl: 'app/template/calendar.tpl.html',
        restrict: 'E',
        transclude: true,
        link: function(scope, element, attrs) {
          var getWeeks, init, selectors, today;
          selectors = document.querySelectorAll('#dateSelectors');
          today = moment();
          scope.month = '';
          scope.year = today.year();
          if (scope.inputClass) {
        	  for(var i=0; i<selectors.length; i++) {
        		  selectors[i].className = selectors[i].className + " " + scope.inputClass;
        	  }
          }
          if (!scope.dateFormat) {
